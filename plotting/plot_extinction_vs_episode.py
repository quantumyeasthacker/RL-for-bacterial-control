# %%
"""Plot mean extinction fraction vs. training episode for the GRU eval sweep.

Consumes the per-checkpoint eval output produced by
scripts/eval/eval_rnn_varenv_gru_ulsweep.sbatch: for each model and each saved checkpoint
(episode_<n>), that sbatch writes 100 trial_*tcbk.pkl rollouts under
"{model_dir}_eval/<trial>/episode_<n>/". Trial dirs are ul<n>-tagged (train_unroll_len),
so this plots the delay1 GRU rnn (unroll 15) and delay5 GRU encoder_decoder (unroll 5) models.

For every checkpoint this script loads those rollouts, computes the extinction
fraction (fraction of trials whose final cell count is 0, matching plot_gen.py:
extinction = 1 if tcbk[1, -1] == 0 else 0), and plots that mean fraction against
the training-episode index, with a +/- 1 std band shaded around each curve. One
curve per model.
...
# %% ----- ----- ----- ----- config ----- ----- ----- ----- %% #
# Loader params. Extinction only uses the final cell count (tcbk[1, -1]); SIM_LENGTH
# just bounds the cell-trajectory padding in load_logger_data_new and does not affect
# the extinction fraction, so a generous upper bound is fine.
SIM_LENGTH = 1000
MAX_POP = int(1e11)
N_TRIALS = 100          # trials per checkpoint written by the sbatch (rep_eval 0..9 x 10)

# The two GRU models evaluated by scripts/eval/eval_rnn_varenv_gru.sbatch. Each model's
# eval output lives next to its own model dir, in "{results_dir}_eval".
MODELS = [
    {
        "label": "delay1 GRU (rnn), unroll 15",
        "eval_dir": Path("/storage/project/r-sbanerjee347-0/jkratz3/pnas_rl/rnn_varenv_runs/results_eval/"
                         "a3.72_T6_delay1_GRU_ul15_rep0"),
        "color": COLOR_LIST[0],
    },
    {
        "label": "delay5 GRU (encoder_decoder), unroll 5",
        "eval_dir": Path("/storage/project/r-sbanerjee347-0/jkratz3/pnas_rl/rnn_varenv_encdec_runs/results_eval/"
                         "a3.72_T6_delay5_GRU_encdec_ul5_rep0"),
        "color": COLOR_LIST[1],
    },
]

OUT_NAME = "extinction_vs_episode_gru"
