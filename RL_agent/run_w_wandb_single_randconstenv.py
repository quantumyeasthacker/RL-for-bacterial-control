...
if MAIN:
    ...
    nutrient_value = [float(i_n) for i_n in sys.argv[2].split("_")]
    delay_embed_len = int(sys.argv[3])
    rep = int(sys.argv[4])
    results_dir = sys.argv[5]

    ## ----- wandb setting ----- ##
    trial_name = f"a{antibiotic_value:.2f}_n{nutrient_value}_delay{delay_embed_len}_rep{rep}"
    ...
    wandb.init(project="antibioticRL-zihang-randconst-nutrient-nutrient-delay-30",
               dir=folder_name,
               name=str(trial_name),
               config=wandb_config,
