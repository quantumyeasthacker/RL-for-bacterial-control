...
def main():
    ...
    metrics = ["log10_final", "log10_max", "burden", "cum_reward", "extinct", "drug_frac", "ext_time"]
    ...
    base = (by_env[(by_env.noise == 0.0) & (by_env.lag == 0)]
            .set_index("group")[["log10_final", "burden", "cum_reward"]])
    ...
    print(f"{'group':12s} {'clean':>8s} {'worst':>8s} {'Δ':>8s}  {'extinct_clean':>13s} {'extinct_worst':>13s}")
    for g in GROUP_ORDER:
        ...
        print(f"{g:12s} {c.log10_final:8.2f} {w.log10_final:8.2f} {w.log10_final-c.log10_final:+8.2f}"
              f"  {c.extinct:13.2f} {w.extinct:13.2f}")
