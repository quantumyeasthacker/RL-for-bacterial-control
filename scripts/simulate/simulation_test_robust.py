...
if MAIN:
    ...
    rep = int(sys.argv[5])
    phiSmax_frac = float(sys.argv[6])
    phiRmax_frac = float(sys.argv[7])
    results_dir = sys.argv[8]

    num_decisions = 300

    cell_config = CellConfig()
    cell_config.phiR_max = phiRmax_frac * cell_config.phiR_max
    cell_config.phiS_max = phiSmax_frac * cell_config.phiS_max
    ...
    folder_name=f"{results_dir}/a{antibiotic_value:.2f}_{eval_env}_{eval_variable}_phiR{phiRmax_frac}_phiS{phiSmax_frac}_value_check/{initialize_app}_{half_period}/"
