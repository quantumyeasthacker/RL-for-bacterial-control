...
if MAIN:
    ...
    const_holdout_string = sys.argv[8]
    const_holdout = [int(i_C) for i_C in const_holdout_string.split("_")]
    # optional trailing arg: "MLP" (default) or "RNN" to train the recurrent RNN_full agent
    agent_type = sys.argv[9].upper() if len(sys.argv) > 9 else "MLP"

    ## ----- wandb setting ----- ##
    trained_env = f"const{constant_nutrient_string}_hold{const_holdout_string}_T{T_k_n0_string}"
    ...
    wandb_auth.init(project="antibioticRL-generalized-agent-holdout_const",
                    dir=folder_name,
                    name=str(trial_name),
                    config=wandb_config,
    ...
    env_config = EnvConfig(
        ...
        k_n0_mean = 2.55,
        sigma_kn0 = 0.1,
        hold_out_range_const = const_holdout
    )
