...
if MAIN:
    ...
    T_k_n0_holdout_string = sys.argv[8]
    T_k_n0_holdout = [int(i_T) for i_T in T_k_n0_holdout_string.split("_")]
    ...
    ## ----- wandb setting ----- ##
    trained_env = f"const{constant_nutrient_string}_T{T_k_n0_string}_hold{T_k_n0_holdout_string}"
    ...
    wandb_auth.init(project="antibioticRL-generalized-agent-holdout_var",
                    dir=folder_name,
                    name=str(trial_name),
                    config=wandb_config,
    ...
    env_config = EnvConfig(
        ...
        k_n0_mean = 2.55,
        sigma_kn0 = 0.1,
        hold_out_range_var = T_k_n0_holdout
    )
