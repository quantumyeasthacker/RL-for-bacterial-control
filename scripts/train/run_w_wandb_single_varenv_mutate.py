"""Single variable-nutrient training run with cell mutation, logged to wandb.

Same as run_w_wandb_single_varenv.py but the cells mutate (phiS_max can change at
each division event), with mutate_prob as an added required positional arg.

Optionally the agent also observes the slow proteome context (population-average phi_S,
re-measured only every context_update_freq decision steps and held fixed in between). This
is an env-side observation block, so it applies to the MLP and RNN agents alike.

Usage:
    python run_w_wandb_single_varenv_mutate.py \
        antibiotic_value T_k_n0 delay_embed_len rep results_dir mutate_prob \
...
Optional positional args:
    ...
    context_update_freq : int, default 0   decision steps between phi_S re-measurements;
    ...
    context_signal   : str, default "phi_S". "phi_S" = the real proteome signal; "noise" = an
                       INFORMATION-FREE CONTROL drawn from the same marginal and latched/aged
                       identically, which separates "phi_S is informative" from "the extra input
                       units helped". Tagged "rand" in the trial name.
...
Output (under results_dir/<trial_name>/):
    ...
    trial_name = a<antibiotic>_T<T>_delay<d>_mutprob<p>_<agent_tag>[_ctx<freq>[noage][rand]]_rep<rep>
"""
...
if MAIN:
    ...
    context_signal   = sys.argv[17]         if len(sys.argv) > 17 else "phi_S"  # can also be "noise" or "phiS_max"
    ...
    trial_name = f"a{antibiotic_value:.2f}_T{T_k_n0}_delay{delay_embed_len}_mutprob{mutate_prob}_{agent_tag}{ctx_tag}_rep{rep}"
    ...
    wandb_auth.init(project="antibioticRL-varenv-mutate",
                    dir=folder_name,
                    name=str(trial_name),
                    config=wandb_config,
