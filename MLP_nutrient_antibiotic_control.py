    ...
    def train(self, num_decisions=250): ###
        ...
        for i in range(self.episode_num,episodes):
            ...
            for k in range(1,36+self.delay_embed_len):
                _, cell_count = self.sim_controller.simulate_population(cell_count[-1], b, k_n0)
                if k >= 36:
            ...
            for j in range(num_decisions):
                ...
                action_kn0 = self.kn0_actions[self.kn0_index[action_index]]

                _, cell_count = self.sim_controller.simulate_population(cell_count[-1], action_b, action_kn0)
