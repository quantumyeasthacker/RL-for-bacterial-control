...
from cell_model_full_parallel import Cell_Population as Cell_Population
from cell_model_no_nutrient_parallel import Cell_Population as Cell_Population_no_nutrient
from cell_model_no_antibiotic_parallel import Cell_Population as Cell_Population_no_antibiotic
...
class CDQL:
    ...
    def __init__(self, delay_embed_len=10,
        ...
        if training_config["agent_input"] == "full":
            self.sim_controller = Cell_Population(num_cells_init, delta_t, omega, kn0_mean, T)
            self.embed_multiplier = 3
        elif training_config["agent_input"] == "no_nut":
            self.sim_controller = Cell_Population_no_nutrient(num_cells_init, delta_t, omega, kn0_mean, T)
            self.embed_multiplier = 2
        elif training_config["agent_input"] == "no_act":
            self.sim_controller = Cell_Population_no_antibiotic(num_cells_init, delta_t, omega, kn0_mean, T)
            self.embed_multiplier = 2
        else:
            raise ValueError("Invalid agent_input value.")
        
        self.folder_name = training_config["folder_name"]
        self.num_decisions = training_config["num_decisions"]
        self.num_eval_decisions = training_config["num_eval_decisions"]
        self.num_evals = training_config["num_evals"]
        self.episodes = training_config["episodes"]
    ...
    def train(self):
        ...
        num_eval_decisions = self.num_eval_decisions
        ...
        for i in range(self.episode_num,episodes):
            ...
            if (i % 10 == 0) or (i == episodes-1):
                self.eval(i, num_decisions=num_eval_decisions, num_evals=num_evals)
                self._save_data(self.folder_name, i)
                if i == episodes-1:
                    plot_reward_Q_loss(self.ave_sum_rewards, self.std_sum_rewards, self.grad_updates, self.loss, self.folder_name,
                                   self.ave_Q1, self.ave_Q2, self.ave_Q1_target, self.ave_Q2_target)

    def eval(self, episode, num_decisions=100, num_evals=5): #200, 50
        """Given trained q networks, generate trajectories
        """
        update_plot = DynamicUpdate(self.delta_t,self.delay_embed_len, self.folder_name)
        print("Evaluation")
        ...
        results = Parallel(n_jobs=10)(delayed(self.rollout)(b_all, cell_count_all, t_all, kn0_all, rewards_all, num_decisions,
