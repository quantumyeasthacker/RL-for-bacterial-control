...
BASE_PATH = Path("/home/zihangw/BacteriaAdaptation/jk_agents/episodic_randinit/")
...
# %% ----- ----- ----- ----- constant sim ----- ----- ----- ----- %% #
env_type = "constenv"
param_file = BASE_PATH / f"param_sim_randinit.txt"
sim_folder = BASE_PATH / f"results_sim_randinit"
...
df_constant_sim.columns = ["half_period", "antibiotic_value", "inst_env", "inst_variable", "randstd"]
df_constant_sim["inst_combination"] = "randstd " + df_constant_sim['randstd'] + " " + df_constant_sim["inst_env"] + " " + df_constant_sim["inst_variable"]
...
for param in param_sim:
    ...
    randstd = float(param[4])

    folder_name = sim_folder / f"a{antibiotic_value}_{param[2]}_{param[3]}_rand_std{randstd}_value_check" / f"{initialize_app}_{half_period}/"
...
# %% ----- ----- ----- ----- generalized eval ----- ----- ----- ----- %% #
param_file = BASE_PATH / f"param_gen_agent_eval_randint.txt"
...
df_generalized_eval.columns = ["antibiotic_value", "trained_env", "delay_embed_len", "inst_env", "inst_variable", "rep", "episodes", "training_episode", "randstd"]

df_generalized_eval["inst_variable"] = df_generalized_eval["inst_variable"].astype(float).astype(int).astype(str)

df_generalized_eval["inst_combination"] = "randstd " + df_generalized_eval['randstd'] + " " + df_generalized_eval["inst_env"] + " " + df_generalized_eval["inst_variable"]
...
for param in param_agent:
    ...
    randstd = float(param[8])

    folder_name = f"{trial_name}_{eval_env}_{eval_variable}_randstd{randstd}"
...
# %% ----- ----- ----- ----- new agent ----- ----- ----- ----- %% #
param_file = BASE_PATH / f"param_epi_agent_eval_randint.txt"
...
df_new_agent_eval.columns = ["antibiotic_value", "trained_env", "delay_embed_len", "inst_env", "inst_variable", "rep", "episodes", "training_episode", "randstd"]

df_new_agent_eval["inst_variable"] = df_new_agent_eval["inst_variable"].astype(float).astype(int).astype(str)
df_new_agent_eval["inst_combination"] = "randstd " + df_new_agent_eval['randstd'] + " " + df_new_agent_eval["inst_env"] + " " + df_new_agent_eval["inst_variable"]
...
# %%
sum_df = df_generalized_eval.groupby(["episodes", "trained_env", "rep"])["eval_log_cell"].mean().reset_index()
...
# %%
rand_std_threshold = 1

mean_df = df_generalized_eval_app.groupby(["episodes", "trained_env", "rep", "randstd"])["log_diff"].mean().reset_index()
mean_df = mean_df.sort_values(by='randstd')
mean_df['randstd'] = mean_df['randstd'].astype(float)
mean_df = mean_df.loc[mean_df["randstd"] < rand_std_threshold].reset_index(drop=True)

mean_df_new_agents = df_new_agent_eval_app.copy()
# .loc[df_new_agent_eval_app["training_episode"] == "episode_499"].copy()
mean_df_new_agents = mean_df_new_agents.groupby(["trained_env", "training_episode", "randstd"])["log_diff"].mean().reset_index()
mean_df_new_agents['randstd'] = mean_df_new_agents['randstd'].astype(float)
mean_df_new_agents = mean_df_new_agents.loc[mean_df_new_agents["randstd"] < rand_std_threshold].reset_index(drop=True)

fig = plt.figure()
plt.scatter(mean_df['randstd'], mean_df['log_diff'], label="multi-env agent", color='k')
plt.plot(mean_df['randstd'], mean_df['log_diff'], color='k')
for trained_env in mean_df_new_agents['trained_env'].unique():
    for training_episode in mean_df_new_agents['training_episode'].unique():
        ...
        plt.scatter(subset['randstd'], subset['log_diff'], label=f"{trained_env} {training_episode}")
        plt.plot(subset['randstd'], subset['log_diff'])
plt.xlabel('randstd', fontsize=14)
...
fig.savefig(Fig_PATH  / "randinit_new_agent.pdf", dpi=600, bbox_inches='tight')

Fig_PATH = BASE_PATH_PLOT / "figures_jpg"
os.makedirs(Fig_PATH, exist_ok=True)
fig.savefig(Fig_PATH / "randinit_new_agent.jpg", dpi=600, bbox_inches='tight')
